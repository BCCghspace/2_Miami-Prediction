...
shore <-opq(bbox = c(xmin, ymin, xmax, ymax)) %>% 
  add_osm_feature("natural", "coastline") %>%
   osmdata_xml(filename = 'shore.osm')

 shore <- sf::st_read('shore.osm', layer = 'lines') %>%
     st_as_sf(coords = c("LON", "LAT"), crs = EPSG:3857, agr = "constant") %>%
  st_transform('ESRI:102658')
...
#       mutate(Legend = "Unioned Buffer"))
 ...
 #Shoreline attempt 1 (might be wrong)
...
#land? bldg?total?
 ...
 #cor test
 cor.test(houses$foodBevNN4, houses$SalePrice, method = "pearson")
