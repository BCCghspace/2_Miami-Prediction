...
#bars, restaurants, shopping
 ...
 #Coastline (doesn't work yet)
shore <-opq(bbox = c(xmin, ymin, xmax, ymax)) %>% 
...
#attempt 2 at shoreline distance (doesn't work yet)
...
ggplot(df,aes(X,Y,fill=shoreDist))+ #variables
 ...
 houses <-
   ...
   mutate(
     ...
     #shops
     shopNN5 = nn_function(st_c(st_centroid(houses)), st_c(shop), 5))
...
#land? bldg?total?
 ...
 #cor test
 cor.test(houses$shopNN5, houses$SalePrice, method = "pearson")
 
ggplot() +
  geom_sf(data = miamiBound) +
  geom_sf(data = flood, aes(fill = FloodZone)) +
